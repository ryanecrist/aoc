...
int main(int argc, char **argv) {
  ...
  const auto & entry = std::max_element(totalMinutesAsleep.begin(), totalMinutesAsleep.end(),
    [](const pair<int, int>& p1, const pair<int, int>& p2) {
      return p1.second < p2.second; });

  int guardId = entry->first;
  int maxMinute = max_element(minutesAsleep[guardId].begin(), minutesAsleep[guardId].end()) 
    - minutesAsleep[guardId].begin();

  cout << guardId * maxMinute << endl;
}
